<template>
  <v-row no-gutters class="HeroRow" @click="goToHeroProfil(hero)">
    ...
    <template v-else>
      ...
      <v-col class="col px-4" style="margin-top: 12px">
        <div class="HeroName">
          {{ hero.name }}
        </div>
      ...
      <v-col
        class="col col-auto column layout"
        style="flex-direction: auto; display: flex; padding-right: 16px"
      >
        <v-btn
        ...
        <v-btn
          v-if="hero.isModified && !hero.isCreated"
          class="mt-4"
          fab
          small
          dark
        ...
        <v-dialog v-model="confirmation" persistent width="500">
          <template v-slot:activator="{ on, attrs }">
            <v-btn class="mt-4" fab small color="red" v-bind="attrs" v-on="on">
          ...
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn class="ConfirmationDelete" text @click="deleteHero(hero)">
