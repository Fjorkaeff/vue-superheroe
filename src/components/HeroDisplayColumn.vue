<template>
  <v-card class="mx-auto my-12" max-width="350" hover>
    <div @click="goToHeroProfil(hero)">
      ...
      <v-card-title>
        <div v-if="hero.name.length > 30">
          {{ hero.name.slice(0, 27) + "..." }}
        </div>
      ...
      <div class="HeroDescripionCard">
        <v-card-text>
          ...
          <div v-else-if="hero.description.length > 90">
            {{ hero.description.slice(0, 90) + "..." }}
          </div>
    ...
    <v-card-actions>
      ...
      <v-dialog v-model="confirmation" persistent width="500">
        <template v-slot:activator="{ on, attrs }">
          <v-btn class="mx-4" fab color="red" v-bind="attrs" v-on="on">
